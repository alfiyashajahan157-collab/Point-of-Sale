...
// Note: table_id field removed as it's not available in Odoo 19
export const fetchOpenOrdersByTable = async (tableId) => {
  try {
    // Since table_id field doesn't exist in Odoo 19, return empty result
    console.warn('fetchOpenOrdersByTable: table_id field not available in Odoo 19');
    return { result: [] };
  } catch (error) {
...
export const createDraftPosOrderOdoo = async ({ sessionId, userId, tableId, partnerId = false, note = '', preset_id = 10 } = {}) => {
  try {
    const vals = {
      ...
      user_id: userId || false,
      partner_id: partnerId || false,
      // table_id: tableId || false, // Removed: table_id field not available in Odoo 19
      lines: [],
      internal_note: note,
      amount_tax: 0,
      amount_total: 0,
  ...
  try {
    ...
    const response = await axios.post(`${ODOO_BASE_URL}/web/dataset/call_kw`, {
      ...
      params: {
        ...
        method: 'search_read',
        args: [domain],
        kwargs: { fields: ['id', 'name', 'state', 'amount_total', 'create_date'], limit, order: 'create_date desc' },
      },
      id: new Date().getTime(),
    }, { headers: { 'Content-Type': 'application/json' } });
    if (response.data && response.data.error) {
